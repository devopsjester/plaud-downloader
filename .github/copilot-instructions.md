# plaud-downloader — Workspace Instructions
...
**Entry point:** `./cmd/plaud-downloader/main.go`
**Output format:** Markdown with YAML front matter, compatible with Obsidian

## Project Layout

```
cmd/plaud-downloader/   CLI entry point
...
- Config file: `{os.UserConfigDir()}/plaud-downloader/plaud-downloader.yaml` (macOS: `~/Library/Application Support/...`)
...
- Config filename is `plaud-downloader.yaml`, not `config.yaml` — read and write paths must match
